...
export function StudyPlanImportDialog({
  ...
  async function analyze() {
    ...
    try {
      ...
      const response =
        await fetch(
          '/api/academic/study-plans/import/preview',
          {
            method: 'POST',
            credentials:
              'include',
      ...
      if (!response.ok) {
        ...
        throw new Error(
          body.message ??
            'No fue posible analizar el plan.',
        );
      }
  ...
  async function confirm() {
    ...
    if (!campusIds.length) {
      toast.error(
        'Selecciona al menos un recinto.',
      );
    ...
    try {
      const response =
        await fetch(
          '/api/academic/study-plans/import/confirm',
          {
            method: 'POST',
      ...
      if (!response.ok) {
        ...
        throw new Error(
          body.message ??
            'No fue posible importar el plan.',
        );
      }

      toast.success(
        'Plan de estudios importado correctamente.',
      );
      ...
      >
        ...
        >
          ...
          <button
            type="button"
            onClick={onClose}
            className="grid size-10 shrink-0 place-items-center rounded-xl hover:bg-slate-100"
          >
            <X className="size-5" />
          </button>
        </header>
            ...
            <>
              ...
              <section className="mt-5 rounded-2xl border p-4 sm:p-5">
                ...
                <div className="mt-3 grid gap-2 sm:grid-cols-2">
                  ...
                  <Validation
                    text={`${preview.detection.newSubjects} asignaturas nuevas`}
                  />
