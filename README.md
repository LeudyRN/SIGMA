...
El resumen general usa `/dashboard/insights` para pendientes actuales, accesos de trabajo y actividad de los últimos 7 días. Reportes es una pantalla independiente: `/dashboard/report-insights` exige `GENERAL_REPORTES_LEER`, admite períodos de 7/30/90 días o personalizados (máximo 366) y ofrece análisis de inscripciones, uso y pagos con tablas y CSV por tema. Los reportes de pagos excluyen deuda actual; los estados de solicitudes se identifican como estado actual, no como cierre histórico. Los eventos auditados no se presentan como consumo total de API/DB. Las fechas se agrupan con el huso horario de Santo Domingo. La conciliación acepta código de método y, por compatibilidad, ID; el formulario envía el código y ofrece únicamente métodos activos.
...
- El cronograma admite actividades para todo el curso o un proyecto específico: inicio, avance, asesoría, defensa y normativa. Las ediciones usan control de versión. Los hitos cancelados permanecen en el calendario.
- Los estudiantes del grupo adjuntan PDF de hasta 8 MB; cada envío conserva una versión. Asesores, evaluadores asignados y coordinación revisan la última versión pendiente, con aprobación o ajustes y retroalimentación obligatoria. Una entrega aprobada se cierra; las versiones antiguas no se sobrescriben. La descarga comprueba acceso en el servidor.
- El seguimiento se actualiza cada 30 segundos. Distingue entregas vencidas, cambios solicitados y espera de revisión; el porcentaje mide avances/defensas aprobados, no asistencia ni calificación final. La respuesta media se calcula sobre las últimas versiones revisadas.
- Las solicitudes docentes indican documento, finalidad e instrucciones. UCOTESIS revisa el último PDF y registra el trámite como enviado y luego completado con referencia. Es control administrativo de designación, contratación o pago; no firma contratos ni ejecuta pagos docentes.
- `AcademicAlertsService` revisa el calendario cada minuto mientras la API está activa, también al arrancar. Crea avisos internos en las 24 horas previas y al alcanzar la fecha, una vez por actividad/versión/destinatario/tipo mediante una clave única. Recupera pendientes al reiniciar; no depende de abrir la pantalla. No envía correo ni WhatsApp. Los trabajos ya entregados y pendientes de revisión no generan avisos de entrega vencida.

La migración aditiva `20260910010000_coordinacion_academica` crea las tablas de perfiles, designaciones, hitos, entregas, expedientes y control de avisos sin modificar migraciones previas. Aplicar `pnpm prisma:deploy` y `pnpm prisma:generate`; actualizar la sesión para cargar los permisos nuevos. `pnpm --filter @sigma/api exec tsx scripts/verify-monograph.ts` también verifica el nuevo recorrido con MySQL y revierte sus fixtures.
